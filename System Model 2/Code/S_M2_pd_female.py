...
mydir_out = '/lustre/home/ucabmc2/workspace/Python/EMD_GP_py/Logs/PD/Dataset_1/SM2/'
mydir_read = '/lustre/home/ucabmc2/workspace/Python/EMD_GP_py/Data/'
mydir_res = '/lustre/home/ucabmc2/workspace/Python/EMD_GP_py/Results/PD/Dataset_1/SM2/'
...
#experiment 1 data:
IMF_train_pd_f = np.load (mydir_read + 'IMF_train_f_pd_d1.npy')
#IMF_train_pd_m = np.load (mydir_read + 'IMF_train_m_pd_d1.npy')
...
N_pd_f = len(IMF_train_pd_f[0])
#N_pd_m = len(IMF_train_pd_m[0])
...
IMF_train_pd_f_seg = [[ IMF_train_pd_f[imf_index][j][ind_sld[i]:ind_sld[i+1]] for i in range(0, len(ind_sld) -1) ] for j in range(ind[0],ind[1])]
...
IMF_train_pd_f_seg_centr = [ [ (np.array(np.vstack(IMF_train_pd_f_seg[j][i])) - np.mean(IMF_train_pd_f_seg[j][i]))/np.std(IMF_train_pd_f_seg[j][i]) for i in range(0,len(IMF_train_pd_f_seg[j]))]  for j in range(0,len(IMF_train_pd_f_seg))]
#IMF_train_pdc_m_seg_centr = [ [ (np.array(np.vstack(IMF_train_pd_m_seg[j][i])) - np.mean(IMF_train_pd_m_seg[j][i]))/np.std(IMF_train_pd_m_seg[j][i]) for i in range(0,len(IMF_train_pd_m_seg[j]))]  for j in range(0,len(IMF_train_pd_m_seg))]
...
if __name__ == '__main__':    #IT WORKS ON THE CLUSTER
     ...
     pool = mp.Pool(4)
     tic = time.perf_counter()
     res = pool.apply( fit_arima_model_pd_2_parallel, args = (IMF_train_pd_f_seg_centr, ind_sld, ind) )
     toc = time.perf_counter()
     pool.close()    
...
np.save(mydir_res +'res_pd_f_'  + str(ind2) + '_imf_' + str(imf_index) , res)
