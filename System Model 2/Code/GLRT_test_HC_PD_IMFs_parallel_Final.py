...
#on Laptop
#mydir_read = 'C:\\Users\\Marta\\Desktop\\Parkinson_data\\Python_code\\Data_extraction_&_preliminaries\\Results\\IMF_train_test\\Test_last_extracted\\'
#mydir_read1 = "C:\\Users\\Marta\\Desktop\\Parkinson_data\\Python_code\\System_M2\Results_Fisher_Kernel\\Best_models\\"


#On Cluster -- modify these
mydir_out = '/lustre/home/ucabmc2/workspace/Python/EMD_GP_py/Logs/Female_GLRT/SM2/'
mydir_read = '/lustre/home/ucabmc2/workspace/Python/EMD_GP_py/Data/IMFs_last_extracted/'
mydir_read1 = '/lustre/home/ucabmc2/workspace/Python/EMD_GP_py/Data/'
mydir_res = '/lustre/home/ucabmc2/workspace/Python/EMD_GP_py/Results/Female_GLRT/SM2/'
...
IMFs_files = os.listdir(mydir_read)

def order(elem):
    return elem.split("_")[2:5]



IMFs_ordered = sorted(IMFs_files, key=order) 

print(IMFs_ordered)

IMF_test = np.load( mydir_read + IMFs_ordered[patient_number] , allow_pickle=True)
...
if gender[patient_number] == 'f':
    if index_imf == 0:
        final_pd_order_rho_IMF_hc = np.load(mydir_read1 + 'final_pd_order_rho_hc_f_imf0.npy',
                                           allow_pickle= True)
        final_pd_order_rho_IMF_pd = np.load( mydir_read1 +'final_pd_order_rho_pd_f_imfs.npy',
                                  allow_pickle = True)
        best_hc_model_imfs = np.load( mydir_read1 +'best_hc_model_f_imf0.npy',
                          allow_pickle=True)
        best_pd_model_imfs = np.load( mydir_read1 +'best_pd_model_f_imf0.npy',
                          allow_pickle=True)
    elif index_imf == 1:
        final_pd_order_rho_IMF_hc = np.load(mydir_read1 + 'final_pd_order_rho_hc_f_imf1.npy',
                                            allow_pickle= True)
        final_pd_order_rho_IMF_pd = np.load( mydir_read1 +'final_pd_order_rho_pd_f_imfs.npy',
                                  allow_pickle = True)
        best_hc_model_imfs = np.load( mydir_read1 +'best_hc_model_f_imf1.npy',
                          allow_pickle=True)
        best_pd_model_imfs =  np.load( mydir_read1 +'best_pd_model_f_imf1.npy',
                          allow_pickle=True)
    elif index_imf == 2:
        final_pd_order_rho_IMF_hc = np.load(mydir_read1 + 'final_pd_order_rho_hc_f_imf2.npy',
                                            allow_pickle= True)
        final_pd_order_rho_IMF_pd = np.load( mydir_read1 +'final_pd_order_rho_pd_f_imfs.npy',
                                  allow_pickle = True)
        best_hc_model_imfs = np.load( mydir_read1 +'best_hc_model_f_imf2.npy',
                          allow_pickle=True)
        best_pd_model_imfs = np.load( mydir_read1 +'best_pd_model_f_imf2.npy',
                          allow_pickle=True)
    elif index_imf == 3:
        final_pd_order_rho_IMF_hc = np.load(mydir_read1 + 'final_pd_order_rho_hc_f_imf3.npy',
                                            allow_pickle= True)  
        final_pd_order_rho_IMF_pd = np.load( mydir_read1 +'final_pd_order_rho_pd_f_imfs.npy',
                                  allow_pickle = True)
        best_hc_model_imfs = np.load( mydir_read1 +'best_hc_model_f_imf3.npy',
                          allow_pickle=True)
        best_pd_model_imfs = np.load( mydir_read1 +'best_pd_model_f_imf3.npy',
                          allow_pickle=True)
    elif index_imf == 4:
        final_pd_order_rho_IMF_hc = np.load(mydir_read1 + 'final_pd_order_rho_hc_f_imf4.npy',
                                            allow_pickle= True)
        final_pd_order_rho_IMF_pd = np.load( mydir_read1 +'final_pd_order_rho_pd_f_imfs.npy',
                                  allow_pickle = True)
        best_hc_model_imfs = np.load( mydir_read1 +'best_hc_model_f_imf4.npy',
                          allow_pickle=True)
        best_pd_model_imfs = np.load( mydir_read1 +'best_pd_model_f_imf4.npy',
                          allow_pickle=True)

elif gender[patient_number] == 'm':
    if index_imf == 0:
        final_pd_order_rho_IMF_hc = np.load(mydir_read1 + 'final_pd_order_rho_hc_m_imf0.npy',
                                            allow_pickle= True)
        final_pd_order_rho_IMF_pd = np.load( mydir_read1 +'final_pd_order_rho_pd_m_imfs.npy',
                                  allow_pickle = True)
        best_hc_model_imfs = np.load( mydir_read1 +'best_hc_model_m_imf0.npy',
                          allow_pickle=True)
        best_pd_model_imfs = np.load( mydir_read1 +'best_pd_model_m_imf0.npy',
                          allow_pickle=True)
    elif index_imf == 1:
       final_pd_order_rho_IMF_hc = np.load(mydir_read1 + 'final_pd_order_rho_hc_m_imf1.npy',
                                            allow_pickle= True)
       final_pd_order_rho_IMF_pd = np.load( mydir_read1 +'final_pd_order_rho_pd_m_imfs.npy',
                                  allow_pickle = True)
       best_hc_model_imfs = np.load( mydir_read1 +'best_hc_model_m_imf1.npy',
                          allow_pickle=True)
       best_pd_model_imfs =  np.load(mydir_read1 + 'best_pd_model_m_imf1.npy',
                          allow_pickle=True)
    elif index_imf == 2:
       final_pd_order_rho_IMF_hc = np.load(mydir_read1 + 'final_pd_order_rho_hc_m_imf2.npy',
                                            allow_pickle= True)
       final_pd_order_rho_IMF_pd = np.load( mydir_read1 +'final_pd_order_rho_pd_m_imfs.npy',
                                  allow_pickle = True)
       best_hc_model_imfs = np.load( mydir_read1 +'best_hc_model_m_imf2.npy',
                          allow_pickle=True)
       best_pd_model_imfs = np.load(mydir_read1 + 'best_pd_model_m_imf2.npy',
                          allow_pickle=True)
    elif index_imf == 3:
       final_pd_order_rho_IMF_hc = np.load(mydir_read1 + 'final_pd_order_rho_hc_m_imf3.npy',
                                            allow_pickle= True)   
       final_pd_order_rho_IMF_pd = np.load( mydir_read1 +'final_pd_order_rho_pd_m_imfs.npy',
                                  allow_pickle = True)
       best_hc_model_imfs = np.load( mydir_read1 +'best_hc_model_m_imf3.npy',
                          allow_pickle=True)
       best_pd_model_imfs = np.load(mydir_read1 + 'best_pd_model_m_imf3.npy',
                          allow_pickle=True)
    elif index_imf == 4:
       final_pd_order_rho_IMF_hc = np.load(mydir_read1 + 'final_pd_order_rho_hc_m_imf4.npy',
                                            allow_pickle= True)
       final_pd_order_rho_IMF_pd = np.load( mydir_read1 +'final_pd_order_rho_pd_m_imfs.npy',
                                  allow_pickle = True)
       best_hc_model_imfs = np.load( mydir_read1 +'best_hc_model_m_imf4.npy',
                          allow_pickle=True)
       best_pd_model_imfs = np.load(mydir_read1 + 'best_pd_model_m_imf4.npy',
                          allow_pickle=True)
...
IMF_test_seg = [[ IMF_test[index_imf][j][ind[i]:ind[i+1]] for i in range(0, len(ind) -1) ] for j in range(0,N_test)] 
...
IMF_test_seg_centr = [  ( IMF_test_seg[i] - np.mean(IMF_test_seg[i]) )/ np.std(IMF_test_seg[i]) for i in range(0, N_seg)] 
...
np.save(mydir_res +'GLRT_pat_' + str(patient_number) +  '_status_' + str(status[patient_number])  + '_' + str(gender[patient_number]) + "_imf_" + str(index_imf) +  '_seg_'   + str(ind2) , GLRT_linear)
np.save(mydir_res +'GLRTnosig_pat_'  + str(patient_number) +  '_status_' + str(status[patient_number])  + '_' + str(gender[patient_number]) + "_imf_" + str(index_imf) +   '_seg_'   + str(ind2)  , GLRT_linear_nosig)
