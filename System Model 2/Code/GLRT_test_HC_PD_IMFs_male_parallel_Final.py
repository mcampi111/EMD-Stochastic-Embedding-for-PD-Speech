# -*- coding: utf-8 -*-
"""
Created on Sat Sep  4 18:05:34 2021

@author: Marta
"""

#########################################
#   GLRT System Model 1  #    IMFs      #
#########################################
########################
#     Male Patients    #
########################
...
#mydir_read2 = "C:\\Users\\Marta\\Desktop\\Parkinson_data\\Python_code\\System_M2\Results_Fisher_Kernel\\Best_models\\"


#On Cluster -- modify these
mydir_out = '/lustre/home/ucabmc2/workspace/Python/EMD_GP_py/Logs/Male_GLRT/SM2/'
mydir_read = '/lustre/home/ucabmc2/workspace/Python/EMD_GP_py/Data/'
mydir_res = '/lustre/home/ucabmc2/workspace/Python/EMD_GP_py/Results/Male_GLRT/SM2/'
...
def calculate_K_GM_linear_no_centering_new(F_score_vec,N_seg):
    ...
    K_id = [ K[i] + (np.identity(8) * (10**(-40)) ) for i in range(0, N_seg)] 
...
# def eval_model_parallel(X_test, model, N_seg, N_mod, ARIMA = ARIMA):
#     eval_model =  [[ ARIMA(X_test[i], order= model[j][0].order ) for i in range(0,N_seg)] for j in range(0,N_mod)]
    
#     return(eval_model)

# def eval_model_parallel(X_test, model, N_seg, N_mod):
#     eval_model = [[model[j].apply(X_test[i]) for i in range(0,N_seg)] for j in range(0,N_mod)]
...
#ind_seg =  np.concatenate( (np.arange(0, 4450, 100), 4450), axis = None)
#ind_seg =  np.concatenate( (np.arange(0, 3750, 100), 3750), axis = None)

#ind_seg = np.concatenate( (np.arange(0, 1000, 20), 1000), axis = None)
...
#IMPORT ONE GROUP OF TESTING SEGMENTS AT A TIME

ind_test_seg_for_res = 0
index_pat_for_res = 0


#IMF_test_hc_f = np.load ( mydir_read +'IMF_test_hc_f.npy')
IMF_test_hc_m = np.load ( mydir_read +'IMF_test_hc_m.npy')
#IMF_test_pd_f = np.load ( mydir_read +'IMF_test_pd_f_d1.npy')
#IMF_test_pd_m = np.load ( mydir_read +'IMF_test_pd_m_d1.npy')
#IMF_test_pd_f_new_pat = np.load ( mydir_read +'IMF_test_pd_f_d1_new_pat.npy')
#IMF_test_pd_m_new_pat = np.load ( mydir_read +'IMF_test_pd_m_d1_new_pat.npy')


############################################
#  LOAD WEIGHTS RHO FROM TRAING SEGMENTS   #
############################################

#if index_imf == 0:
#    final_pd_order_rho_IMF_hc_f = np.load(mydir_read + 'final_pd_order_rho_hc_f_imf0.npy',
#                                           allow_pickle= True)
#elif index_imf == 1:
#    final_pd_order_rho_IMF_hc_f = np.load(mydir_read + 'final_pd_order_rho_hc_f_imf1.npy',
#                                            allow_pickle= True)
#elif index_imf == 2:
#    final_pd_order_rho_IMF_hc_f = np.load(mydir_read + 'final_pd_order_rho_hc_f_imf2.npy',
#                                            allow_pickle= True)
#elif index_imf == 3:
#    final_pd_order_rho_IMF_hc_f = np.load(mydir_read + 'final_pd_order_rho_hc_f_imf3.npy',
#                                            allow_pickle= True)    
#elif index_imf == 4:
#    final_pd_order_rho_IMF_hc_f = np.load(mydir_read + 'final_pd_order_rho_hc_f_imf4.npy',
#                                            allow_pickle= True)
...
#final_pd_order_rho_IMF_pd_f = np.load( mydir_read +'final_pd_order_rho_pd_f_imfs.npy',
#                                  allow_pickle = True)



if index_imf == 0:
     final_pd_order_rho_IMF_hc_m = np.load(mydir_read + 'final_pd_order_rho_hc_m_imf0.npy',
                                            allow_pickle= True)
elif index_imf == 1:
     final_pd_order_rho_IMF_hc_m = np.load(mydir_read + 'final_pd_order_rho_hc_m_imf1.npy',
                                             allow_pickle= True)
elif index_imf == 2:
     final_pd_order_rho_IMF_hc_m = np.load(mydir_read + 'final_pd_order_rho_hc_m_imf2.npy',
                                             allow_pickle= True)
elif index_imf == 3:
     final_pd_order_rho_IMF_hc_m = np.load(mydir_read + 'final_pd_order_rho_hc_m_imf3.npy',
                                             allow_pickle= True)    
elif index_imf == 4:
     final_pd_order_rho_IMF_hc_m = np.load(mydir_read + 'final_pd_order_rho_hc_m_imf4.npy',
                                             allow_pickle= True)
...
final_pd_order_rho_IMF_pd_m = np.load( mydir_read +'final_pd_order_rho_pd_m_imfs.npy',
                                      allow_pickle = True)


############################################################
#  LOAD BEST MODELS and PARAMETERS from the TRAINING SETS  #
############################################################

#if index_imf == 0:
#    best_hc_model_f_imfs = np.load( mydir_read +'best_hc_model_f_imf0.npy',
#                          allow_pickle=True)
#elif index_imf == 1:
#     best_hc_model_f_imfs = np.load( mydir_read +'best_hc_model_f_imf1.npy',
#                          allow_pickle=True)
#elif index_imf == 2:
#     best_hc_model_f_imfs = np.load( mydir_read +'best_hc_model_f_imf2.npy',
#                          allow_pickle=True)
#elif index_imf == 3:
#     best_hc_model_f_imfs = np.load( mydir_read +'best_hc_model_f_imf3.npy',
#                          allow_pickle=True)
#elif index_imf == 4:
#     best_hc_model_f_imfs = np.load( mydir_read +'best_hc_model_f_imf4.npy',
#                          allow_pickle=True)


#if index_imf == 0:
#    best_pd_model_f_imfs = np.load( mydir_read +'best_pd_model_f_imf0.npy',
#                          allow_pickle=True)
#elif index_imf == 1:
#     best_pd_model_f_imfs =  np.load( mydir_read +'best_pd_model_f_imf1.npy',
#                          allow_pickle=True)
#elif index_imf == 2:
#     best_pd_model_f_imfs = np.load( mydir_read +'best_pd_model_f_imf2.npy',
#                          allow_pickle=True)
#elif index_imf == 3:
#     best_pd_model_f_imfs = np.load( mydir_read +'best_pd_model_f_imf3.npy',
#                          allow_pickle=True)
#elif index_imf == 4:
#     best_pd_model_f_imfs = np.load( mydir_read +'best_pd_model_f_imf4.npy',
#                          allow_pickle=True)
                      


if index_imf == 0:
     best_hc_model_m_imfs = np.load( mydir_read +'best_hc_model_m_imf0.npy',
                           allow_pickle=True)
elif index_imf == 1:
      best_hc_model_m_imfs = np.load( mydir_read +'best_hc_model_m_imf1.npy',
                           allow_pickle=True)
elif index_imf == 2:
      best_hc_model_m_imfs = np.load( mydir_read +'best_hc_model_m_imf2.npy',
                           allow_pickle=True)
elif index_imf == 3:
      best_hc_model_m_imfs = np.load( mydir_read +'best_hc_model_m_imf3.npy',
                           allow_pickle=True)
elif index_imf == 4:
      best_hc_model_m_imfs = np.load( mydir_read +'best_hc_model_m_imf4.npy',
                           allow_pickle=True)


if index_imf == 0:
     best_pd_model_m_imfs = np.load( mydir_read +'best_pd_model_m_imf0.npy',
                           allow_pickle=True)
elif index_imf == 1:
      best_pd_model_m_imfs =  np.load('best_pd_model_m_imf1.npy',
                           allow_pickle=True)
elif index_imf == 2:
      best_pd_model_m_imfs = np.load('best_pd_model_m_imf2.npy',
                           allow_pickle=True)
elif index_imf == 3:
      best_pd_model_m_imfs = np.load('best_pd_model_m_imf3.npy',
                           allow_pickle=True)
elif index_imf == 4:
      best_pd_model_m_imfs = np.load('best_pd_model_m_imf4.npy',
                           allow_pickle=True)
                      


IMF_test_hc_m_seg = [[ IMF_test_hc_m[index_imf][j][ind[i]:ind[i+1]] for i in range(0, len(ind) -1) ] for j in range(0,N_f_test)] 
IMF_test_hc_m_seg = [item for sublist in IMF_test_hc_m_seg for item in sublist] 
IMF_test_hc_m_seg = IMF_test_hc_m_seg[ind_s[0] : ind_s[1]]

N_seg = len(IMF_test_hc_m_seg)

#IMF_test_pd_m_seg = [[ IMF_test_pd_m[index_imf][j][ind[i]:ind[i+1]] for i in range(0, len(ind) -1) ] for j in range(0,N_f_test)] 
#IMF_test_pd_m_seg = [item for sublist in IMF_test_pd_m_seg for item in sublist] 
#IMF_test_pd_m_seg = IMF_test_pd_f_seg[ind_s[0] : ind_s[1]]

#N_seg = len(IMF_test_pd_m_seg)
...
#IMF_test_hc_f_seg_centr = [  ( IMF_test_hc_f_seg[i] - np.mean(IMF_test_hc_f_seg[i]) )/ np.std(IMF_test_hc_f_seg[i]) for i in range(0, N_seg)] 

IMF_test_hc_m_seg_centr = [  ( IMF_test_hc_m_seg[i] - np.mean(IMF_test_hc_m_seg[i]) )/ np.std(IMF_test_hc_m_seg[i]) for i in range(0, N_seg)] 
...
#                                                    N_seg, N_mod, arma_scoreobs, np, match ) )
...
if __name__ == '__main__':    
     ...
     U_0_final = pool.apply( score_function_eval_hc, args = (IMF_test_hc_m_seg_centr, best_hc_model_m_imfs, final_pd_order_rho_IMF_hc_m,
                                                             my_param_names, N_seg, N_mod, arma_scoreobs, match) )
     U_1_final = pool.apply( score_function_eval_pd, args = (IMF_test_hc_m_seg_centr, best_pd_model_m_imfs, final_pd_order_rho_IMF_pd_m,
                                                             my_param_names, N_seg, N_mod, arma_scoreobs, match) )
...
np.save(mydir_res +'GLRT_m_test_status_' + str(ind_test_seg_for_res)  + '_pat_' + str(index_pat_for_res)   + str(ind2) +'_imf_' + str(index_imf) , GLRT_linear)
np.save(mydir_res +'GLRT_nosig_m_test_status_' + str(ind_test_seg_for_res)  + '_pat_' + str(index_pat_for_res)    + str(ind2) +'_imf_' + str(index_imf) , GLRT_linear_nosig)
