...
mydir_out = '/lustre/home/ucabmc2/workspace/Python/EMD_GP_py/Logs/PD/'
...
ind_seg = np.concatenate( (np.arange(0, N_pd_m, 30), N_pd_m), axis = None)
...
#IMF_train_pd_f_centr = [ (np.array(np.vstack(IMF_train_pd_f[imf_index][i])) - np.mean(IMF_train_pd_f[imf_index][i]))/np.std(IMF_train_pd_f[imf_index][i]) for i in range(ind[0],ind[1])] # 50 250
IMF_train_pd_m_centr = [ (np.array(np.vstack(IMF_train_pd_m[imf_index][i])) - np.mean(IMF_train_pd_m[imf_index][i]))/np.std(IMF_train_pd_m[imf_index][i])  for i in range(ind[0],ind[1])] # N_hc_m
...
if __name__ == '__main__':    #IT WORKS ON THE CLUSTER
     ...
     pool = mp.Pool(4)
     tic = time.perf_counter()
     res = pool.apply( fit_arima_model_parallel, args = (IMF_train_pd_m_centr, ind_sld, ind) )
     toc = time.perf_counter()
     pool.close()    
...
np.save(mydir_res +'res_pd_m_'  + str(ind2) + '_imf_' + str(imf_index) , res)
