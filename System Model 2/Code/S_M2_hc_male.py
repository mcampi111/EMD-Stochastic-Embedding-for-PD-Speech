# -*- coding: utf-8 -*-
"""
Created on Sat Aug 28 18:45:37 2021
...
#IMF_train_hc_f_centr = [ (np.array(np.vstack(IMF_train_hc_f[imf_index][i])) - np.mean(IMF_train_hc_f[imf_index][i]))/np.std(IMF_train_hc_f[imf_index][i]) for i in range(ind[0],ind[1])] # 50 250
IMF_train_hc_m_centr = [ (np.array(np.vstack(IMF_train_hc_m[imf_index][i])) - np.mean(IMF_train_hc_m[imf_index][i]))/np.std(IMF_train_hc_m[imf_index][i])  for i in range(ind[0],ind[1])] # N_hc_m
...
# - normal - #
###############
#res = fit_arima_model_parallel(IMF_train_hc_m_centr, ind_sld, ind_seg)
###################
# - in parallel - #
####################
#pool = mp.Pool(2)
...
###################################################
# FOR IMF4 AND IMF 5 NEED TO ADAPAT THE FOLLWOING #
###################################################
#prova = [auto_arima(IMF_train_hc_m_centr[0][ind_sld[i]:ind_sld[i+1]], 
#                    error_action='ignore',
#                    start_p=0, max_p= 1,
#                    start_q= 0, max_q= 1,
#                    d = 1, max_d = 1, m = 1,
#                    seasonal = False) for i in range(0,len(ind_sld)-1)] 
#plt.plot(IMF_train_hc_m_centr[0][101:200])
...
if __name__ == '__main__':    #IT WORKS ON THE CLUSTER
     ...
     pool = mp.Pool(4)
     tic = time.perf_counter()
     res = pool.apply( fit_arima_model_parallel, args = (IMF_train_hc_m_centr, ind_sld, ind) )
     toc = time.perf_counter()
     pool.close()    
...
np.save(mydir_res +'res_hc_m_'  + str(ind2) + '_imf_' + str(imf_index) , res)
